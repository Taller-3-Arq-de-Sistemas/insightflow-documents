...
class DocumentService {
  ...
  async createDocument(data) {
    ...
    const WORKSPACE_SERVICE_URL =
      "https://insightflow-workspaces.onrender.com/api/workspaces";

    try {
      const response = await fetch(
        `${WORKSPACE_SERVICE_URL}/${data.workspace_id}`
      );

      if (response.status === 404) {
        throw new Error(`El Workspace ID "${data.workspace_id}" no existe.`);
      }

      if (response.status === 400) {
        throw new Error(`El Workspace ID "${data.workspace_id}" no es válido`);
      }
    } catch (error) {
      if (
      ...
      console.warn(
        `Advertencia: No se pudo verificar el workspace (${error.message}). Creando documento de todas formas.`
      );
    }
